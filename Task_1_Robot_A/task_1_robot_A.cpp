...
int main()
{
	...
	*/
	...
	open_video_output("task_1_robot_A.mp4");
	int t_position = 0;
	...
	while (1) {
		...
		if (iterations_number >= 100) {
			...
			*/
			...
			position green_centroid(green_x, green_y);
			position red_centroid(red_x, red_y);
			...
			*/
			...
			//cout << "\nAlpha: " << alpha;
			//cout << "\nCHECK B";
			int* shoot_orange_target;


			//cout << "\nIterations since last time laser was fired: " << iterations_number - iterations_number_shoot;
			shoot_orange_target = shooting(cur, position(orange_x, orange_y), angcur, alpha);
			if ((shoot_orange_target[0] == 1) && (iterations_number - iterations_number_shoot >= 50))
			{
				laser = shoot_orange_target[0];
				pw_laser = shoot_orange_target[1];
				shooting_avail =  0;
