...
public:
	...
	static bool obs_bool_shooting(position p1, position p2) {
		double p1_p2_angle = angle_between_points(p1, p2);
		double safety_coefficient = 42; // +2 just to make sure we don't hit the obstacle
		bool obs_bool_shooting = 0;
		bool shooting_capability = 1;
		double distance_line_center;
		double m; // angular coefficient of the line connecting the two points p1, p2
		double b; // bias term for the line connecting p1, p2
		double distance, distance_0, distance_cent_p1;
		...
		for (int j = 0; j < obstacle_cent.size(); j++) // check if the line connecting the two points (cur and destination) contains the centroid of an obstacle
		{
			...
			dist_max = (position::distance_between_points(p1, p2));
			...
			if ((abs(distance_line_center) < safety_coefficient) && ((distance_between_points(p1, obstacle_cent[j]) * cos(difference_angles)) <= dist_max) && (difference_angles <= PI / 2)) {
				obs_bool_shooting = 1;
				shooting_capability = 0;
			}
		}

		//cout << "\nCan the robot shoot, given the position of the obstacles? --> " << 1 - obs_bool_shooting;


		return obs_bool_shooting;
	}
...
void obstacle_points()
{
	...
	int radius = 40; // 2 is used as a safety factor
...
void calculate_next_position(position cur, position destination)
{
	...
	double step = 10;
	double intermediates_shift_margin_initial = 100.0;
	int shift_position_margin = intermediates_shift_margin_initial;
...
int* shooting(position cur, position target, double angle_cur, double alpha) {
	static int shoot[2];
	double epsilon_shooting = 0.05;
	bool obs_onway_shooting = position::obs_bool_shooting(cur, target);
	double desired_angle = position::angle_between_points(cur, target);
	double angle_difference = (constrain_angle_negpi_pi(angle_cur + alpha) - constrain_angle_negpi_pi(desired_angle));
	double difference_angles_aux;
	difference_angles_aux = angle_difference;
	
	// Shooting check

	if ((abs(difference_angles_aux) < epsilon_shooting) && (obs_onway_shooting == 0)) {
		//cout << " ---> SHOOT!";
		shoot[0] = 1;
		shoot[1] = 1500; 
		return shoot;
	}
	else if ((abs(difference_angles_aux) >= epsilon_shooting) && ((obs_onway_shooting == 0) || (obs_onway_shooting == 1))) {
		//cout << " ---> ROTATING SHOOTING TURRET...";
		if ((abs(difference_angles_aux) > 0.0) && (abs(difference_angles_aux) <= PI)) {
			shoot[1] = 1700;
		}
		else {
			shoot[1] = 1300;
...
int* rotation_control(position cur, position dest, double angle_cur) {
	...
	double difference_angles_aux;
	double difference_angles;
	difference_angles_aux = (constrain_angle_negpi_pi(position::angle_between_points(cur, dest)) - constrain_angle_negpi_pi(angle_cur));

	double epsilon = 0.02;
	if (((difference_angles_aux) < -PI) || (difference_angles_aux) > 0.0) {
		pw[1] = pw_0 + pw_add;
		pw[0] = pw_0 + pw_add;
	}
	else {
...
int main()
{
	...
	int mode;
	int pw_l_o = 1500, pw_r_o = 1500, pw_laser_o = 1500, laser_o = 0;
	int* result;
	bool available = 0;
	bool within_reach = 0;
	double epsilon = 0.01;
	static double distance_centroids_robot_aux;
	static double distance_centroids_robot = 40; // found by displaying "distance_centroids_robot_aux", try also 80
	static int iterations_number_shoot = 0;
	bool shooting_avail = 0;
	double alpha;
	int capture = 0;
	int v_mode;
	i2byte nlabel = 0;
	position opponent_pos;
	position green_centroid;
	position red_centroid;
	robot* pointer_robot_alpha;
	...
	*/
	...
	activate_simulation(width1,height1,x_obs,y_obs,N_obs,
		"robot_B.bmp","robot_A.bmp","background.bmp", obstacle_file, D,Lx,Ly,
	...
	x0 = 500;
	y0 = 150;
	theta0 = PI / 2; // initial orientation of the robot
	...
	double t_duration = 60;
	open_video_input("input1_480p.mp4", t_duration, width, height);
	open_video_output("task_1_robot_B.mp4");
	int t_position = 0;
	...
	while (1) {
		...
		if (iterations_number >= 100) {
			...
			// keep rgb unaltered, use rgb0 for processing
			...
			if (initial == 0) // do it only once, it's an initialization process
			{
				...
				cout << "\nObstacle # 1: --> " << obstacle_cent[0].x << "," << obstacle_cent[0].y;
				cout << "\nObstacle # 2: --> " << obstacle_cent[1].x << "," << obstacle_cent[1].y;
				copy(rgb, rgb0);
			...
			for (int i = 0; i < (obstacle_cent.size()); i++) {
				draw_point_rgb(rgb, obstacle_cent[i].x, obstacle_cent[i].y, R_obs, G, B_obs);
			}
			...
			/*
			for (int i = 0; i < (obstacles_obstruction_points.size()); i++) {
				draw_point_rgb(rgb, obstacles_obstruction_points[i].x, obstacles_obstruction_points[i].y, R_obs, G, B_obs);
			}
			...
			*/
			...
			// 
			//cout << "\nGreen centroid: --> " << green_x << "," << green_y;
			//cout << "\nRed centroid: --> " << red_x << "," << red_y;
			//cout << "\nOrange centroid: --> " << orange_x << "," << orange_y;
			//cout << "\nBlue centroid: --> " << blue_x << "," << blue_y;

			// print the points belonging to the obstacles
			...
			double angcur = (atan2(orange_y - blue_y, orange_x - blue_x));

			// the current position is calculated wrt the middle between the two centroids
			double x_cur = (orange_x);
			double y_cur = (orange_y);

			position orange_centroid(orange_x, orange_y);
			position blue_centroid(blue_x, blue_y);

			// calculate the distance between the two centroids of the robot

			distance_centroids_robot_aux = position::distance_between_points(green_centroid, red_centroid);
			// cout << "\nDistance between centroids: " << distance_centroids_robot_aux;

			opponent_pos.x = (red_x + green_x) / 2;
			opponent_pos.y = (red_y + green_y) / 2;

			position cur(x_cur, y_cur);
			position dest = opponent_pos;


			if ((position::distance_between_points(cur, dest) <= 120) && (abs(constrain_angle_negpi_pi(position::angle_between_points(cur, dest)) - constrain_angle_negpi_pi(angcur)) >= epsilon)) {
				within_reach = 1;
				result = rotation_control(cur, dest, angcur);
				pw_l = result[0];
				pw_r = result[1];
				//cout << "\nCurrent angle difference: " << constrain_angle_negpi_pi(position::angle_between_points(cur, dest)) - constrain_angle_negpi_pi(angcur);
				//cout << "\nRobot too close and NOT aligned";
			}

			else if ((position::distance_between_points(cur, dest) <= 120) && (abs(constrain_angle_negpi_pi(position::angle_between_points(cur, dest)) - constrain_angle_negpi_pi(angcur)) < epsilon)) {
				within_reach = 1;
				pw_l = 1500;
				pw_r = 1500;
				//cout << "\nCurrent angle difference: " << constrain_angle_negpi_pi(position::angle_between_points(cur, dest)) - constrain_angle_negpi_pi(angcur);
				//cout << "\nRobot too close and aligned";
			}

			if (position::distance_between_points(cur, dest) > 120) {
				within_reach = 0;
			}

			if ((next_destination_queue.size() == 0) && (within_reach == 0)) {
				available = 1;
			}

			if (next_destination_queue.size() != 0) {
				draw_point_rgb(rgb, next_destination_queue[0].x, next_destination_queue[0].y, R_obs, G, B_obs);
			}

			//cout << "\nIs the opponent too close? " << within_reach;

			/*
			cout << "\nCurrent robot position: " << " -> " << "x :" << xcur << " , y :" << ycur;
			cout << "\nCurrent opponent position: " << " -> " << "x :" << opponent_pos.x << " , y :" << opponent_pos.y;
			cout << "\nAngle between the two robots: " << position::angle_between_points(cur, dest);
			*/
			//cout << "\nDestination queue size: " << destination_queue.size();

			//cout << "\nIs the robot available for computation? --> " << available;
			
			// Shooting

			// we shoot only the sandwitch box
			//cout << "\nCHECK A";
			alpha = S1->P[1]->x[4];
			//cout << "\nAlpha: " << alpha;
			//cout << "\nCHECK B";
			int* shoot_green_target;


			//cout << "\nIterations since last time laser was fired: " << iterations_number - iterations_number_shoot;
			shoot_green_target = shooting(cur, position(green_x, green_y), angcur, alpha);
			if ((shoot_green_target[0] == 1) && (iterations_number - iterations_number_shoot >= 50))
			{
				laser = shoot_green_target[0];
				pw_laser = shoot_green_target[1];
				shooting_avail =  0;
				iterations_number_shoot = iterations_number;
			}
			else
			{
				laser = 0;
			...
			if (iterations_number % 50 == 0) {
				cout << "\nOpponent right wheel input: " << pw_r_o;
				cout << "\nOpponent left wheel input: " << pw_l_o;
			}
			set_inputs(pw_l, pw_r, pw_laser, laser, max_speed);
			...
			double t = high_resolution_time() - tc0;
			// write image to video file
			write_video_output(rgb, t);
			...
			save_rgb_image("output_attacker.bmp", rgb);
		}
	...
	deactivate_vision(rgb, rgb0, rgb, a, b, c, label, labeled);
